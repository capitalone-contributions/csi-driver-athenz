...
import (
	...
	"github.com/AthenZ/csi-driver-athenz/internal/approver/evaluator"
)
...
// Athenz CSI driver and evaluates whether they should be approved or denied.
...
func (a *approver) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := a.evaluator.Evaluate(&cr); err != nil {
		...
		apiutil.SetCertificateRequestCondition(&cr, cmapi.CertificateRequestConditionDenied, cmmeta.ConditionTrue, "csi.cert-manager.athenz.io", "Denied request: "+err.Error())
		return ctrl.Result{}, a.client.Status().Update(ctx, &cr)
	}

	log.Info("approving request")
	apiutil.SetCertificateRequestCondition(&cr, cmapi.CertificateRequestConditionApproved, cmmeta.ConditionTrue, "csi.cert-manager.athenz.io", "Approved request")
	return ctrl.Result{}, a.client.Status().Update(ctx, &cr)
}
